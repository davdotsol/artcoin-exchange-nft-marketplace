...
describe('NFTMarket Contract', function () {
  ...
  async function marketplaceFixture() {
    ...
    const tx = await nftMarketplace
      .connect(addr1)
      .listNFT(nftMarket, 0, ethers.parseEther('0.05'));
    await tx.wait();
  ...
  describe('Marketplace', function () {
    it('Should list an NFT correctly', async function () {
      const { tx, nftMarketplace, nftMarket, owner, addr1 } = await loadFixture(
        marketplaceFixture
      );
      ...
      await expect(tx)
        ...
        ); // Correct the index for args

      // Verify the nftItem details
      const nftItem = await nftMarketplace.getNFTItem(nftMarket.target, 0);
    ...
    it('Should allow buying an NFT', async function () {
      ...
      const txBuyNFT = await nftMarketplace
        .connect(addr2)
        .buyNFT(nftMarket.target, 0, { value: ethers.parseEther('0.05') });
      // Verify the event was emitted correctly
    ...
    it('Should revert if the price is incorrect', async function () {
      ...
      await expect(
        nftMarketplace
          .connect(addr2)
          .buyNFT(nftMarket.target, 0, { value: ethers.parseEther('0.001') })
    ...
    it('Should correctly add and remove owned NFTs', async function () {
      ...
      await nftMarketplace
        .connect(addr2)
        .listNFT(nftMarket, 0, ethers.parseEther('0.1'));
      ...
      await nftMarketplace
        .connect(addr1)
        .buyNFT(nftMarket.target, 0, { value: ethers.parseEther('0.1') });
