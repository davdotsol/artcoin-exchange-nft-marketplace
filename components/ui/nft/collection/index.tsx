...
import { useCollectionNFTs, useCollectionStatistics } from '@hooks/web3';
...
export default function Collection({ collectionAddress }: CollectionProps) {
  const router = useRouter();
  const { nfts } = useCollectionNFTs(collectionAddress);
  const { statistics } = useCollectionStatistics(collectionAddress);
      ...
      <div className="relative bg-primary pt-16 pb-20 px-4 sm:px-6 lg:pt-24 lg:pb-28 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="mt-10 text-center">
            ...
            <p>Total Listings: {statistics?.totalListings}</p>
            <p>Total Sales: {statistics?.totalSales}</p>
          </div>
          <div className="mt-12 max-w-lg mx-auto grid gap-5 lg:grid-cols-3 lg:max-w-none">
            {nfts.data
              ?.filter((nft: NFTItem) => nft.isListed)
              .map((nft: NFTItem) => (
                <div
                  key={nft.meta.image}
